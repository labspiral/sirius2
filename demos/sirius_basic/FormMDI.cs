...
namespace Demos
{
    public partial class FormMDI : Form
    {
        ...
        #region Example for auto focus
        double targetFocusPosition = 0;
        double optimalFocusPosition;
        private void mnuAutoFocus_Click(object sender, EventArgs e)
        {
            var camera = Vision.Control.Camera;
            if (camera.IsBusy)
                return;

            double minFocusPosition = 0;  // Edit to minimum focus position
            double maxFocusPosition = 100; // Edit to maximum focus position
            int numSteps = 20; // Edit position steps
            optimalFocusPosition = minFocusPosition;
            bestScore = -1;

            bool success = true;
            camera.OnImageGrabbed += Camera_OnImageGrabbed;
            try
            {
                for (int i = 0; i <= numSteps; i++)
                {
                    targetFocusPosition = minFocusPosition + i * (maxFocusPosition - minFocusPosition) / numSteps;
                    success &= MoveToPosition(targetFocusPosition); // Adjust camera/lens focus position
            ...
            if (success)
            {
                Logger.Log(Logger.Types.Warn, $"trying to move best focus position= {optimalFocusPosition}");
                MoveToPosition(optimalFocusPosition);
            }
        }

        double bestScore = -1;
        private bool Camera_OnImageGrabbed(ICamera camera, Bitmap bitmap, IProcess process, object state)
        {
            double score = CogHelper.CalculateSharpnessScore(bitmap);
            // Get highest score position
            if (score > bestScore)
            {
                bestScore = score;
                optimalFocusPosition = this.targetFocusPosition;
                Logger.Log(Logger.Types.Debug, $"updated best focus score= {score}, position= {optimalFocusPosition}");
            }
            return true;
        }
        private bool MoveToPosition(double position)
        {
