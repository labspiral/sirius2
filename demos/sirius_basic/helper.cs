 ...
 * Description : editor / vision helper
...
namespace Demos
{
    ...
    public static class SiriusHelper
    {
        ...
        /// <returns>Success or failed</returns>
        public static bool CreateDevices(out IRtc rtc, out IDInput dInExt1, out IDInput dInLaserPort, out IDOutput dOutExt1, out IDOutput dOutExt2, out IDOutput dOutLaserPort, out ILaser laser, out IPowerMeter powerMeter, out IMarker marker, out IRemote remote, out ICamera camera, out IInspector inspector, int index = 0)
        {
            rtc = null;
            dInExt1 = null;
            dInLaserPort = null;
            ...
            if (null != rtc2ndHead)
            {
                ...
                // or 
                //rtc.CtlMatrix(ScannerHeads.Primary,
                //    System.Numerics.Matrix3x2.CreateScale(1, 1) *
                //    System.Numerics.Matrix3x2.CreateRotation((float)(angle1 * Math.PI / 180.0)) *
                //    System.Numerics.Matrix3x2.CreateTranslation(dx1, dy1));
                ...
                if (0 != enable2ndHead)
                {
                    ...
                    // or
                    //rtc.CtlMatrix(ScannerHeads.Secondary,
                    //  System.Numerics.Matrix3x2.CreateScale(1, 1) *
                    //  System.Numerics.Matrix3x2.CreateRotation((float)(angle2 * Math.PI / 180.0)) *
                    //  System.Numerics.Matrix3x2.CreateTranslation(dx2, dy2));
                }
            }
            ...
            double mmPerPixel = camera.PixelSize / camera.LensMagnification;
            camera.UnitTransform = new UnitTransformBase(mmPerPixel, mmPerPixel);
            ...
            {
                camera.UnitTransform.XmmPerPixel = NativeMethods.ReadIni<double>(ConfigFileName, $"CAMERA{index}", "TRANFORM_X_MM_PER_PIXEL", mmPerPixel);
                camera.UnitTransform.YmmPerPixel = NativeMethods.ReadIni<double>(ConfigFileName, $"CAMERA{index}", "TRANFORM_Y_MM_PER_PIXEL", mmPerPixel);
            ...
            if (!string.IsNullOrEmpty(calibrationFile))
            {
                ...
                if (CenterCalibratorSerializer.Open(calibrationPath, out var cell))
                    camera.CenterCalibrator = cell;
            }
        ...
        /// <returns>Success or failed</returns>
        public static bool CreateTestEntities(IRtc rtc, SpiralLab.Sirius2.Winforms.IDocument document)
        {
        ...
        /// <summary>
        /// Create test processes at IDocument 
        /// </summary>
        ...
        public static void CreateTestProcesses(SpiralLab.Sirius2.Vision.IDocument doc)
        {
            ...
            doc.ActAdd(proc5);
            var proc6 = ProcessFactory.CreateBarcode();
            doc.ActAdd(proc6);

            var layer = ProcessFactory.CreateLayer("Stitch");
        ...
        private static void Camera_OnCalibratorSaved(ICamera camera, CenterCalibrator cell, string fileName)
        {
        ...
        private static void CameraStitched_OnStitchedCalibratorSaved(ICameraStitched cameraStitched, ProcessCalibrator process, string fileName)
        {
